...
def CalcDamageRate(Main):
    ...
    DamageWindow.geometry("180x250+%d+%d"%(Main.w+Main.x,Main.y))
    DamageWindow.configure(bg='blue')

    #증가 데미지 수치
    DamageUpLabel = Label(DamageWindow,text="증뎀 : ",bg='blue',fg="yellow")
    DamageUpLabel.place(x=10,y=20)
    DamageUp = Entry(DamageWindow,width=10)
    DamageUp.place(x=50,y=20)
    Label(DamageWindow, text="%",bg='blue',fg="yellow").place(x=130, y=20)

    #크리티컬 증가 데미지
    CDamageUpLabel = Label(DamageWindow, text="크증 : ",bg='blue',fg="yellow")
    CDamageUpLabel.place(x=10, y=60)
    CDamageUp = Entry(DamageWindow,width=10)
    CDamageUp.place(x=50, y=60)
    Label(DamageWindow, text="%",bg='blue',fg="yellow").place(x=130, y=60)

    #스킬 공격력 증가
    SDamageUpLabel = Label(DamageWindow, text="스증 : ",bg='blue',fg="yellow")
    SDamageUpLabel.place(x=10, y=100)
    SDamageUp = Entry(DamageWindow,width=10)
    SDamageUp.place(x=50, y=100)
    Label(DamageWindow, text="%",bg='blue',fg="yellow").place(x=130, y=100)

    #모든 공격력 증가
    ADamageUpLabel = Label(DamageWindow, text="모공 : ",bg='blue',fg="yellow")
    ADamageUpLabel.place(x=10, y=140)
    ADamageUp = Entry(DamageWindow,width=10)
    ADamageUp.place(x=50, y=140)
    Label(DamageWindow, text="%",bg='blue',fg="yellow").place(x=130, y=140)

    #추가 데미지 증가
    AddDamageUpLabel = Label(DamageWindow, text="추뎀 : ",bg='blue',fg="yellow")
    AddDamageUpLabel.place(x=10, y=180)
    AddDamageUp = Entry(DamageWindow, width=10)
    AddDamageUp.place(x=50, y=180)
    Label(DamageWindow, text="%",bg='blue',fg="yellow").place(x=130, y=180)
    ...
    def Skillinfo():
        ...
        for i in range(len(skillinfo['skill']['style']['active'])):
            ...
            Label(SkillWindow,text=skillinfo['skill']['style']['active'][i]['name']+" : "+str(int(damagelist[i]))+"K").place(x=0,y=i*30)



    Total = Button(DamageWindow, text='계산시작', bg='blue', fg="yellow")
    ...
    def Click(Status,skill):
        ...
        if (Status['status'][2]['value'] > Status['status'][3]['value']):
            TotalDamage = temp * SDamage * 0.01 * (Status['status'][2]['value'] * 0.004 * Status['status'][6]['value']) * (
                    Status['status'][23]['value'] - 20) * 0.0045 * (1.5 + CDamage) * (1+ADamage) * (1+AddDamage)
...
def Switcinginfo(Main):
    ...
    for i in range(len(Equip['skill']['buff']['equipment'])):
        ...
        tempImg = PhotoImage(data=ItemImageData[i])
        switchlabels.append(Label(SwitchWindow,image=tempImg))
        switchtextlabels.append(Label(SwitchWindow,text=Equip['skill']['buff']['equipment'][i]['itemName']))
        switchlabels[i].configure(image=tempImg)
        switchlabels[i].image = tempImg
        switchlabels[i].place(x=0,y=i*30)
        switchtextlabels[i].place(x=30,y=5+i*30)
...
def Iteminfo(Main):
    ...
    SubWindow.geometry("250x400")
    SubWindow.title("아이템 상세 정보")
    SubWindow.iconbitmap("icon.ico")
    labels = list()
    for i in range(len(Equip['equipment'])):
        labels.append(Label(SubWindow,text=Equip['equipment'][i]['slotName'] + " : " + Equip['equipment'][i]['itemName']+" 강화 : " + str(Equip['equipment'][i]['reinforce'])))
